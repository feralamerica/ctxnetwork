...
(function () {
  ...
  var FEEDS = {
    substack: {
      ...
      limit: 6,
      viewAll: "https://feralamerica.substack.com/s/project-545",
    },
  };
  ...
  function load(feedKey) {
    ...
    var lockedByCache = false;
    function apply(items, fromCache) {
      if (fromCache && items && items.length) { lockedByCache = true; best = items; render(feedKey, items); return; }
      if (!lockedByCache && items && items.length > best.length) { best = items; render(feedKey, items); }
    }
    function settle() { settled++; if (settled >= 2 && best.length === 0) render(feedKey, []); }
    cacheFetch(feed).then(function (x) { apply(x, true); settle(); }, function () { settle(); });
    liveFetch(feed).then(function (x) { apply(x, false); settle(); }, function () { settle(); });
  }
